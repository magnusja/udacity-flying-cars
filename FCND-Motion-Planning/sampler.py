...
class Sampler(object):
    def __init__(self, data):
        ...
        centers = np.array([p.center for p in self.polygons])
    ...
    def sample(self, num_samples=300):
        ...
        pts = []
        for s in samples:
            indices = list(self._tree.query_radius(np.array([s[0], s[1]]).reshape(1, -1), r=self._max_poly_xy)[0])
            
            in_collision = False
            if len(indices) > 0:
                for ind in indices: 
                    p = self.polygons[int(ind)]
